...
> Issues with status modified in the last `duration` (from `r start`)
...
issue_table_running <-
  issue_table %>% group_by(type) %>% mutate(count = case_when(
...
ggplot(issue_table_running, aes(event, running_total, linetype = type)) +
