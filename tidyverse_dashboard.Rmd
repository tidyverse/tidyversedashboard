...
---
title: "`r paste0(tidyversedashboard::get_org_name(orgs[[1]]), " repository dashboard")`"
---
...
issue_progress_running %>%
  group_by(maintainer) -> t1 %>%
  filter(n() > 10) -> t2
ggplot(ungroup(t2), aes(event, running_total, linetype = type, color = maintainer)) +
  geom_step() + labs(x = NULL, y = "Open Issues", color = "type")
```
