...
30 day issue progress
=====================================
```{r weekly_summary, cache = on_macos, include = FALSE}
issue_data <- bind_rows(
  issue_progress("tidyverse", start),
  issue_progress("r-lib", start),
  issue_progress("r-dbi", start))

issue_data2 <- weekly_issue_data %>% 
  mutate(
...
> Issues with status modified in the last 30 days.
